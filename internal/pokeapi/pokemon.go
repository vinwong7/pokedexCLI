...
func (c *Client) PokemonStats(name string) (Pokestat, error) {
	...
	if !ok {
		...
		if err != nil {
			return Pokestat{}, fmt.Errorf("error making request")
		}
		...
		if err := json.Unmarshal(res_data, &pokemon_stat); err != nil {
			return Pokestat{}, fmt.Errorf("unmarshal error")
		}

		//return the Pokestat struct to be used for the catch command
		return pokemon_stat, nil

	}

	//Create an empty Pokestat struct, then unmarshal data onto it
	cache_pokemon_stat := Pokestat{}
	if err := json.Unmarshal(cache_data, &cache_pokemon_stat); err != nil {
		return Pokestat{}, fmt.Errorf("unmarshal error")
	}

	//return the Pokestat struct to be used for the catch command
	return cache_pokemon_stat, nil
